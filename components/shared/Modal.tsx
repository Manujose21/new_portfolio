...
export const Modal = ({ children, isModalOpen, closeModal }: { children: React.ReactNode, isModalOpen: boolean, closeModal:() => void}) => {
    ...
    return (
        <div className={`absolute top-0 left-0 bottom-0 right-0 w-full h-full bg-background-primary bg-opacity-30 flex items-center justify-center z-50 ${isModalOpen ? 'scale-100' : 'scale-0'} transition-all duration-150 ease-in-out`}>
